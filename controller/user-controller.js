...
async  function homepageget(req, res) {
    ...
    productHelpers.getLatestProducts().then((products) => {
      categoryHelpers.getAllCategory().then((category) => {
        res.render('user/view-products', { products, category, user, guest: true,cartCount,banners });
      });
    });
  }
 ...
 loginpost= (req, res) => {
  ...
  async function allproductsget(req, res, next) {
    ...
    if (filter) {
      res.render('user/allproducts', { products: filterbyCategory, user, guest: true, filterbyCategory, category });
    } else {
      
      productHelpers.getAllProductsPagination(val)
        .then((products) => {
          res.render('user/allproducts', { products, user, guest: true, filterbyCategory, category });
        })
        .catch((error) => {
          // Handle error
        });
...
viewOrderProducts=async(req,res)=>{
  ...
  res.render('user/view-order-products',{user:req.session.user,products,orders})
}
...
module.exports={homepageget,loginget,signupget,signuppost,otppost,loginpost,logoutget,forgotPasswordget,
                 addtocartget,productDetailsget,allproductsget,cartget,cartaddtocartget,
                 changeProductQuantitypost,removeFromCartpost,placeorderget,placeorderpost,
                 ordersuccessget,orderSummaryget ,viewOrderProducts,cancelorder,addNewAddressPost,
                 useExist,userprofileget,userprofilepost,edituseraddress,edituseraddresspost,
                 deleteAddressget,deleteuserAddressget,verifyPaymentpost,userhomepagepost,errorpageget,
                 allproductspost,applyCouponPost,returnOrder,useWalletPost,
                 addWhishlist,removeWhishlist,userWhishlist,walletget,forgotPost}