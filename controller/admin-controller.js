...
function adminhomepageget(req, res, next) {
 ...
 addproductsget= async (req, res) => {
  ...
  const currentDate = new Date(); 
   ...
   productHelpers.addProduct({No:No, Name:Name,Category:Category,Price:Price,
   Description:Description,photos:photos,date:currentDate}, (id) => {
 ...
 dashbordget = (req, res) => {
  Promise.all([
    ...
    .then(([latestorder, latestuser, totaluser, totalproduct,
      totalAmount, monthlydata, productSells, paymentCounts, sales]) => {
...
userdataget=  async (req, res) => {
 ...
 unblockuserget= (req, res) => {
  ...
  addcategoryget=(req, res) => {
    res.render('admin/add-category', { catError: req.session.catError ,admin:true});
...
function addCategorypost(req, res) {
 ...
 console.log(req.files);
   ...
   const photos=req.files.map((file)=>{
    ...
    categoryHelpers.addCategory({Category:Category,photos:photos}).then((data)=>{
      if(data){
 ...
 deleteCategoryget= (req, res) => {
  ...
  editCategoryget= async (req, res) => {
    ...
    console.log(category);
    res.render('admin/edit-category', { category, admin: true, catError: req.session.catError });
    req.session.catError = false;
  }
  ...
  async function editCategorypost(req, res) {
    try {
      ...
      // Replace the existing photo with the newly uploaded photo, if available
      const updatedPhotos = photos.filter((photo) => photo !== null).length > 0 ? photos : [];
...
salesReport =async(req,res) => {
  ...
  const [latestorder,latestuser,totaluser,totalproduct,
  totalAmount,monthlydata,paymentCounts]=await Promise.all([
  adminHelpers.getAllLatestOrders(),
  adminHelpers.getAllLatestUsers(),
  adminHelpers.totalUser(),
  adminHelpers.totalProduct(),
...
// sales report end//
  ...
  module.exports={adminhomepageget,adminhomepagepost,addproductsget,addproductspost,editProductget,
                  editProductpost,unhideproductget,hideproductget,dashbordget,adminloginget,adminloginpost,
                  adminlogoutget,userdataget,userdatapost,blockuserget, unblockuserget,categoryget,addcategoryget,
                  addCategorypost,deleteCategoryget,editCategoryget,editCategorypost, hideCategoryget,unhideCategoryget,
                  userOrderget,statuschangeget,userorderpost,bannerget,addbannerget,addbannerpost,AddBannerget,deletebannerget,
                  viewCouponGet,addCouponGet,addCouponPost,editCouponGet,editCouponPost,deleteCouponPost,errorpageget,
                  salesReport,salesReports,dateFilterPost}